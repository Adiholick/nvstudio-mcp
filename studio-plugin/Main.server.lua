	...
	if kind == "status" then
		...
		if ui then
			...
			elseif not newActive and changed then
				ui:addLog("[System] ⚠️ IDE Agent terputus.", Color3.fromRGB(255, 150, 50))
			end
		end
		return
	end
...
function startConnectionLoop()
	...
	if ui then
		ui:addLog("[System] NVStudio MCP v2.1.7 — Menghubungkan ke server...", Color3.fromRGB(200, 200, 200))
	end
	...
	while isRunning and not isManualDisconnect do
		...
		if streamOk and streamRaw and streamRaw ~= "" then
			...
			-- Toleransi 2x gagal sebelum menandai server unreachable
